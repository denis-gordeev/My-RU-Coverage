...
1. `## 業務簡介` — краткое описание бизнеса и метаданные.
2. `## 供應鏈位置` — позиция в цепочке поставок.
3. `## 主要客戶及供應商` — ключевые клиенты и поставщики.
4. `## 財務概況` — финансовый блок, который обновляется скриптами.
...
- Для новых материалов основной язык описаний: русский.
- Финансовый раздел не редактируется вручную.
- Единицы зависят от рынка: `.ME` -> `млн руб.`, `.TW/.TWO` -> legacy `百萬台幣`.
