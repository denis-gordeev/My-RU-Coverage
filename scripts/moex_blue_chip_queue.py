"""
moex_blue_chip_queue.py — Проверка официальных корзин МосБиржи через ИСС.
...
def построить_отчёт_по_индексу(код_индекса, элементы):
    переведённые = [_перевести_элемент(э) for э in элементы]
    покрытые = set(найти_файлы_тикеров().keys())
    отсутствующие = [элемент for элемент in переведённые if элемент["тикер"] not in покрытые]
    отчёт = {
        "код_индекса": код_индекса,
        "название_индекса": МЕТКИ_ИНДЕКСОВ.get(код_индекса, код_индекса),
        "дата_торгов": переведённые[0]["дата_торгов"] if переведённые else None,
        "количество": len(переведённые),
        "покрыто": len(переведённые) - len(отсутствующие),
...
def главный():
    ...
    парсер = создать_русский_парсер(
        description=(
            "Проверить актуальный состав официальных корзин МосБиржи через ИСС МосБиржи "
