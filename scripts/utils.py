...
def является_кириллическим_именем(строка):
    ...
    число_кириллических = sum(1 for c in строка if "\u0400" <= c <= "\u04FF")
...
def построить_таблицу_мультипликаторов(оценка):
    ...
    значения = [оценка.get(h, "Н/Д") for h in заголовки]
    ширины = [max(len(h), len(val)) for h, val in zip(заголовки, значения)]
    строка_заголовка = "| " + " | ".join(h.rjust(w) for h, w in zip(заголовки, ширины)) + " |"
    строка_разделителя = "|" + "|".join("-" * (w + 2) for w in ширины) + "|"
    строка_значений = "| " + " | ".join(val.rjust(w) for val, w in zip(значения, ширины)) + " |"
