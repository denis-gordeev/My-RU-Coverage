...
def main():
    ...
    элементы = отобрать_элементы_очереди(отчёт, [тикер.upper() for тикер in args.тикеры])
    if not элементы:
        print("Очередь уже покрыта: новых карточек для создания нет.")
        return

    лимит = None if args.все_непокрытые or args.тикеры else max(args.лимит, 0)
    print(
        f"Генерирую базовые MOEX-карточки из очереди {', '.join(коды_индексов)} "
        f"на дату {отчёт['дата_торгов']}..."
    )
    создано, пропущено = создать_отчёты(элементы, лимит=лимит, пробный_запуск=args.пробный_запуск)
