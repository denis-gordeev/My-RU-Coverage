...
def искать_в_отчётах(ключевое_слово, фильтр_секторов=None):
    ...
    for папка_сектора in sorted(os.listdir(ДИРЕКТОРИЯ_ОТЧЁТОВ)):
        ...
        for f in sorted(os.listdir(путь_сектора)):
...
def main():
    ...
    if "--сектор" in args:
        ...
        if индекс + 1 < len(args):
            фильтр_секторов = {args[индекс + 1]}
    elif "--секторы" in args:
        индекс = args.index("--секторы")
        if индекс + 1 < len(args):
            фильтр_секторов = set(строка.strip() for строка in args[индекс + 1].split(","))
    elif умный:
