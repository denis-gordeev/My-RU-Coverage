...
⚠ УСТАРЕЛО: этот скрипт генерирует отчёты для тайваньского корпуса (.TW/.TWO).
Для российского рынка используйте generate_moex_reports.py.
"""
...
**Сектор:** {sector}
**Отрасль:** {industry}
**Рыночная капитализация:** {market_cap:,.0f} млн тайв. долл.
**Стоимость предприятия (EV):** {enterprise_value:,.0f} млн тайв. долл.
