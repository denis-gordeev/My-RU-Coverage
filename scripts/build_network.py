"""
build_network.py — Строит данные и ХТМЛ-визуализацию графа викилинков.
...
def сканировать_граф(минимальный_вес=5, лимит_узлов=None):
    ...
    for тикер, набор_викилинков in викилинки_по_файлу.items():
        ...
        for и in range(len(отфильтрованные)):
            for ж in range(и + 1, len(отфильтрованные)):
                рёбра[(отфильтрованные[и], отфильтрованные[ж])] += 1
...
def построить_хтмл(узлы, связи):
    """Генерирует автономную ХТМЛ-визуализацию на D3.js."""
...
def главный():
    ...
    for и, аргумент in enumerate(аргументы):
        if аргумент == "--мин-вес" and и + 1 < len(аргументы):
            минимальный_вес = int(аргументы[и + 1])
        elif аргумент == "--лимит" and и + 1 < len(аргументы):
            лимит_узлов = int(аргументы[и + 1])
    ...
    путь_джсон = os.path.join(ДИРЕКТОРИЯ_СЕТИ, "graph_data.json")
    with open(путь_джсон, "w", encoding="utf-8") as ф:
        json.dump(данные_графа, ф, ensure_ascii=False, indent=2)
    print(f"Сохранён файл: {путь_джсон}")

    текст_хтмл = построить_хтмл(узлы, рёбра)
    путь_хтмл = os.path.join(ДИРЕКТОРИЯ_СЕТИ, "index.html")
    with open(путь_хтмл, "w", encoding="utf-8") as ф:
        ф.write(текст_хтмл)
    print(f"Сохранён файл: {путь_хтмл}")

    print(f"\nОткройте в браузере: {путь_хтмл}")
