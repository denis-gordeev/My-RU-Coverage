...
- [ ] Обогатить свежесозданные карточки `DOMRF`, `AKRN`, `AFLT`, `CBOM`, `BSPB`: добавить русскоязычное описание бизнеса, положение в цепочке поставок, клиентов/поставщиков и тематические `[[wikilinks]]`.
- [ ] Продолжить следующую подочередь `MOEXBMI` после первой пятёрки: `AFKS`, `ENPG`, `CNRU`, `BANEP`, `ASTR`.
- [ ] Продолжить перевод `themes/` от mixed-навигации к преобладающе российскому набору на уровне самих тематических страниц, включая появление новых RU-тем и дальнейшее оттеснение legacy-блоков из верхней навигации.
...
- [ ] Открытые issues: на 2026-04-05 `gh issue list` недоступен без авторизации (`HTTP 401`), поэтому актуальное состояние не подтверждено в этом окружении.
- [ ] Открытые PR: на 2026-04-05 `gh pr list --json number,title,headRefName,state` недоступен без авторизации (`HTTP 401`), поэтому актуальное состояние не подтверждено в этом окружении.
