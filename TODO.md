...
- [ ] Доверифицировать нижнюю часть актуальной корзины `MOEXBC` после top-10/factsheet и ноябрьской ребалансировки, чтобы синхронизировать длинную очередь ликвидных эмитентов без опоры на устаревшие legacy-предположения.
- [ ] Разобраться с источником финансовых данных для `YDEX`, `T` и `OZON`: текущий `yfinance` не создал им финансовые блоки и метаданные автоматически, как ранее уже случалось с `TATN`.
...
- [ ] Открытые issues: на 2026-04-03 `gh issue list` недоступен без авторизации (`HTTP 401`), поэтому актуальное состояние не подтверждено в этом окружении.
- [ ] Открытые PR: на 2026-04-03 `gh pr list --json number,title,headRefName,state` недоступен без авторизации (`HTTP 401`), поэтому актуальное состояние не подтверждено в этом окружении.
