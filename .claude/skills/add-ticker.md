---
name: add-ticker
description: Добавить новый тикер в базу покрытия с финансовыми данными и обогащением
user_invocable: true
---

# Добавление тикера

Добавляет новую компанию, торгующуюся на Мосбирже (MOEX), в базу покрытия. Генерирует .md отчёт с финансовыми данными, затем исследует и обогащает его.

## Использование

- `/add-ticker SBER Сбер` — автоопределение сектора через yfinance
- `/add-ticker SBER Сбер --sector Финансовые услуги` — указать сектор вручную

## Инструкции

### Шаг 1: Генерация базового файла

```bash
python scripts/add_ticker.py <тикер> <название> [--sector <сектор>]
```

Создаёт .md файл с метаданными и финансовыми данными из yfinance (суффикс `.ME` для MOEX), а также секции-заглушки для обогащения.

### Шаг 2: Исследование и обогащение

После генерации исследуйте компанию:
1. Веб-поиск: `[Тикер] инвесторские отношения годовой отчёт`, `[Тикер] годовая отчёт`, `[Компания] инвестиционная презентация`
2. **ПРОВЕРКА**: название компании в файле совпадает с результатами исследования (Золотое правило CLAUDE #2)
3. Запишите данные обогащения в формате JSON:

```json
{
  "SBER": {
    "описание": "Описание деятельности с [[викилинками]]...",
    "цепочка_поставок": "**Верхний контур:**\n- ...\n**Ключевое звено:**\n- ...\n**Конечный спрос:**\n- ...",
    "клиенты_и_поставщики": "### Ключевые клиенты\n- ...\n\n### Ключевые поставщики\n- ..."
  }
}
```

4. Сохраните во временный файл и примените:
```bash
python scripts/update_enrichment.py --data /tmp/enrich.json <тикер>
```

### Шаг 3: Аудит

```bash
python scripts/audit_ru_reports.py 2>&1 | grep <тикер>
```

Или прочитайте файл и проверьте соответствие золотым правилам (10+ викилинков, нет обобщений, нет английского).
